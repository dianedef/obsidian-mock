...
export const getAllTags = vi.fn((cache) => {
    const frontmatterTags = cache.frontmatter?.tags || [];
    const inlineTags = cache.tags?.map(t => t.tag) || [];
    return [...new Set([...frontmatterTags, ...inlineTags])];
});
export const getBlobArrayBuffer = vi.fn(async (blob) => {
    return blob.arrayBuffer();
});
export const getFrontMatterInfo = vi.fn((content) => {
...
export const Keymap = {
    isModifier: vi.fn((_evt, _modifier) => {
        return false;
    }),
    isModEvent: vi.fn((_evt) => {
        if (!_evt)
            return false;
        if (_evt.ctrlKey || _evt.metaKey)
            return 'tab';
        if (_evt instanceof MouseEvent && _evt.button === 1)
            return 'tab';
        return false;
    })
};
export const requireApiVersion = vi.fn((version) => {
...
export const resolveSubpath = vi.fn((cache, subpath) => {
    if (!subpath)
        return null;
    // Handle headers
    ...
    if (subpath.startsWith('#')) {
        ...
        if (heading) {
            const nextHeading = cache.headings?.find(h => h.position.start.line > heading.position.start.line &&
...
export const sanitizeHTMLToDom = vi.fn((html) => {
